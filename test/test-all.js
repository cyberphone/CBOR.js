...
file:String.raw`// Testing unread elements operations
...
function reducedOneTurn(length, value, result, exact) {
  let ok = length != null;
  if (!ok) length = 2
  try {
    let reduced = length == 2 ? CBOR.Float.createFloat16(value, exact) : CBOR.Float.createFloat32(value, exact);
    assertTrue("Should not", ok);
...
reducedOneTurn(null, Number.NaN,             0,                      false);
...
,
{name:'nesting.js',
file:String.raw`// Testing nesting checking
