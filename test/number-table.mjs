...
//  console.log(cbor.toString());
   oneTurn(cbor.toString(), CBOR.toHex(encoding), (largest ? "Largest" : "Smallest") +
       " positive " + (subnormal ? 'subnormal ' : "") +
        codeWord('float' + (size * 8)));
}
...
if (intMode) {
  ...
  oneTurn("-5.960464477539063e-8", "f98001",
    "Smallest negative subnormal " + codeWord("float16"));

  oneTurn("-5.960464477539062e-8", "fbbe6fffffffffffff",
     "Adjacent smallest negative subnormal " + codeWord("float16"));

  oneTurn("-5.960464477539064e-8", "fbbe70000000000001", text);

  oneTurn("-5.960465188081798e-8", "fab3800001", text);

  oneTurn("0.0000609755516052246", "fb3f0ff7ffffffffff",
    "Adjacent largest subnormal " + codeWord("float16"));
  ...
  oneTurn("0.00006103515624999999", "fb3f0fffffffffffff", 
    "Adjacent smallest " + codeWord("float16"));

  oneTurn("0.00006103515625000001", "fb3f10000000000001", text);

  oneTurn("0.00006103516352595761", "fa38800001", text);
  
  oneTurn("65503.99999999999", "fb40effbffffffffff",
    "Adjacent largest " + codeWord("float16"));
  ...
  oneTurn("1.4012984643248169e-45", "fb369fffffffffffff", 
    "Adjacent smallest subnormal " + codeWord("float32"));

  oneTurn("1.4012984643248174e-45", "fb36a0000000000001", text);

  oneTurn("1.175494210692441e-38", "fb380fffffbfffffff", 
    "Adjacent largest subnormal " + codeWord("float32"));

  oneTurn("1.1754942106924412e-38", "fb380fffffc0000001", text);

  oneTurn("1.1754943508222874e-38", "fb380fffffffffffff", 
    "Adjacent smallest " + codeWord("float32"));

  oneTurn("1.1754943508222878e-38", "fb3810000000000001", text);

  oneTurn("3.4028234663852882e+38", "fb47efffffdfffffff",
